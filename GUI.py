...
class GUI(Connection, FileOperations):
    pass
    ...
    def IPWindow(self):
        ...
        """ptText = Label(master, text="Enter PORT Number")
        ptText.pack()
        ptText.place(relx=.5, rely=0.40, anchor="center")

        ptTextBox = Text(master, height=1, width=4)
        ptTextBox.pack()
        ptTextBox.place(relx=.5, rely=.55, anchor="center")"""
    ...
    def RootWindow(self):
        ...
        L1B2 = Button(text="View Send Images", command=lambda: [self.Fop.OpenImage(root, path)])
        L1B2.place(relx=.818, rely=0.040)

        L1B3 = Button(text="View Received Images", command=lambda: [self.Fop.OpenImage(root, path + "ReceivedFiles")])
        ...
        L3B1 = Button(text="    Send    ", command=lambda: self.AlertWindow(self.Fop.SendFile(self.Conn)))
        L3B1.place(relx=.470, rely=.950, anchor="center")

        L3B2 = Button(text="  Receive  ", command=lambda: self.AlertWindow(self.Fop.ReceiveFile(self.Conn)))
